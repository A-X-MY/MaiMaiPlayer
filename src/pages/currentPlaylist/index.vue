...
const deleteSong = (row) => {
	const resultIdx = musicstore.songs.findIndex(item => item.id == row.id)
	musicstore.songs.splice(resultIdx, 1)
}
// 添加导入本地歌曲函数
const importMusic = () => {
...
// 新增方法 showPlayedSongs
const showPlayedSongs = () => {
	// 获取已经播放过的歌曲列表 playedSongs 并按照时间排序
	const playedSongs = musicstore.playedSongs.sort((a, b) => new Date(b.playedAt) - new Date(a.playedAt)
	)
// 将排序后的结果赋值给 tableData
	state.tableData = playedSongs
}
...
<template>
	<div class="content-section">
		<div class="button-wrapper">
			...
			<el-button type="primary" size="medium" @click="showPlayedSongs">
				最近播放
			</el-button>
		</div>
				...
				<el-table-column>
					<template #default="scope">
						<div class="button-wrapper">
							...
							<svg @click="deleteSong(scope.row)" t="1686302672906" class="icon" viewBox="0 0 1024 1024"
